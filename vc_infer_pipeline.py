...
bh, ah = signal.butter(N=9, Wn=64, btype="high", fs=16000)
...
class VC(object):
    ...
    def vc(
        ...
        if protect <= 1 and pitch != None and pitchf != None:
            feats0 = feats.clone()
        if (
    ...
    def pipeline(
        ...
        audio_max = np.abs(audio_opt).max() / ((volume-1)/100)
        max_int16 = 32768
        if audio_max > 1:
            max_int16 /= audio_max
